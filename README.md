...
- `open-vm-tools` built **from source** with the patch. The patch applies to the **13.0.5** and **13.1.0** source layouts; other `open-vm-tools` versions likely need a rebase. It is *not* tied to a nixpkgs release; see [Verified versions](#verified-versions).
...
| `open-vm-tools` | 13.0.5 | 13.0.5 deployed, **13.1.0** also verified |
...
**`open-vm-tools` 13.1.0** (currently `nixos-unstable`) is covered by the same patch. It applies with no fuzz and builds on both `x86_64-linux` and `aarch64-linux`, and the round trip was verified on the guest in the table above by running the patched 13.1.0 `vmtoolsd -n vmusr` in place of the deployed 13.0.5 one: host → guest, guest → host, and UTF-8 (accents, arrows, CJK, emoji) all survive intact.

### Re-verifying after an upgrade

```sh
nix eval --raw nixpkgs#open-vm-tools.version   # 13.0.5 or 13.1.0? if not, the patch needs a rebase
...
The `Makefile.am` hunk deliberately anchors on the `if LINUX` block rather than on the neighbouring copy/paste source lines, and adds its two sources at the top level rather than next to them. 13.1.0 moved the X11 sources into `if HAVE_GTK4` / `else` branches, and an anchor there would have broken (or worse, applied with fuzz into the branch that is not always taken, silently dropping the backend). The Wayland files link no Gtk, so top level is also where they belong. Please keep them there.
...
`checks.<system>.patch-target` fails when nixpkgs moves off every `open-vm-tools` release the patch applies to (`patchTargetVersions` in `flake.nix`). `checks.<system>.module-eval` evaluates a minimal VMware guest and pins the module's behaviour, including the headless split described above. Neither compiles anything, so both finish in seconds.

The weekly build job additionally asserts that the built `libdndcp.so` really contains the Wayland backend. Applying cleanly is not the same as reaching the binary: GNU patch allows fuzz, so a hunk can land in a conditional that is not taken and drop the backend while the build still succeeds.
...
**You don't have to match clipway's nixpkgs.** The overlay patches *your* `open-vm-tools` (`prev.open-vm-tools`), so what matters is the `open-vm-tools` version in your nixpkgs, not clipway's. Clipway's own `nixpkgs` input only backs the `packages` output (`nix build`, cache population). It currently tracks `nixos-26.05`; consuming clipway from a 25.11 or unstable system is fine as long as `open-vm-tools` is one of the versions the patch applies to.

**Pinning a release.** The URL above follows `main`, and your `flake.lock` pins a revision either way, so tags aren't needed for reproducibility. `main` aims to cover every `open-vm-tools` release its supported channels ship, so following it is normally the right thing. Tags exist for the case where that coverage has to be dropped, when `main` rebases the patch forward and stops applying to the release your channel is still on:

```nix
inputs.clipway.url = "github:krisztianfekete/clipway/v0.1.0";
```

Every [release](https://github.com/krisztianfekete/clipway/releases) states the `open-vm-tools` versions its patch applies to, so a pinned tag keeps you on a patch that matches your nixpkgs. Releases are cut when something consumer-visible changes: a patch rebase, a `services.clipway.*` option change, or a new [verified version](#verified-versions).
